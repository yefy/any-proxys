    ...
    pub async fn accept(
        ...
        loop {
            ...
            let ret: Result<Option<(Stream, SocketAddr, SocketAddr)>> = async {
                ...
                log::info!(
                    "accept flag:server, min_stream_cache_size:{:?}",
                    tunnel_hello
                );
                ...
                if peer_client.is_some() {
                    ...
                    if peer_client.is_none() {
                        return Err(anyhow!("session_id closed:{}", session_id));
                    }
                    let peer_client = peer_client.unwrap();

                    #[cfg(feature = "anytunnel-debug")]
                    log::info!("accept flag:server, hello_to_peer_stream",);
                    peer_client
                ...
                #[cfg(feature = "anytunnel-debug")]
                log::info!("accept flag:server, create_stream_and_peer_client",);
