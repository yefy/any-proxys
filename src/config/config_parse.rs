...
pub fn merger(mut config: config_toml::ConfigToml) -> Result<config_toml::ConfigToml> {
    ...
    let mut tcp_server_func = || -> Result<()> {
        ...
        if _port.fast.is_none() {
            _port.fast = Some(config.fast.clone());
        }
        ...
        for v in _port._server.iter_mut() {
            ...
            if v.fast.is_none() {
                v.fast = Some(_port.fast.clone().unwrap())
            }
    ...
    let mut domain_server_func = || -> Result<()> {
        ...
        if _domain.fast.is_none() {
            _domain.fast = Some(config.fast.clone());
        }
        ...
        for v in _domain._server.iter_mut() {
            ...
            if v.fast.is_none() {
                v.fast = Some(_domain.fast.clone().unwrap());
            }
    ...
    let mut upstream_server_func = || -> Result<()> {
        ...
        for ups_config in config.upstream.as_mut().unwrap().iter_mut() {
            for ups_server in ups_config.server.iter_mut() {
                match ups_server {
                    config_toml::ProxyPass::Tcp(tcp) => {
                        if tcp.tcp.is_none() {
                            tcp.tcp = Some(config.tcp[0].tcp_name.clone())
