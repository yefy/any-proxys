                ...
                if v.len() >= 3 && &v[0..2] == "$$" {
                    ...
                    if v == "(.*)" {
                        if full_match_index.is_some() {
                            return Err(anyhow::anyhow!("err:domain exist => domain:{}", v));
                        }
                    ...
                    let func = |vars_str: &str| -> Result<(String, String)> {
                        ...
                        if var_start_index.is_none() {
                            return Err(anyhow::anyhow!("err:domain => domain:{}", vars_str));
                        }
                    ...
                    if domain_regex_map.get(&data).is_some() {
                        return Err(anyhow::anyhow!("err:domain => domain:{}", v));
                    }
                    ...
                    if domain_map.get(v).is_some() {
                        return Err(anyhow::anyhow!("err:domain exist => domain:{}", domain));
                    }
    ...
    pub fn index(&self, domain: &str) -> Result<i32> {
        ...
        match domain_index {
            ...
            None => {
                ...
                let find = vars.rfind(".");
                if find.is_none() {
                    return Err(anyhow::anyhow!("err:domain not found => domain:{}", domain))?;
                }
