...
use crate::config::config_toml::ProxyPass;
...
impl UpstreamConfig {
    ...
    pub async fn parse_config(
        ...
        HashMap<String, Rc<RefCell<Option<UpstreamData>>>>,
    )> {
        ...
        let mut ups_configs = Vec::new();
        if config.upstream.is_some() {
            ups_configs = config.upstream.clone().take().unwrap();
        }

        let mut upstream_name_map = HashMap::new();
        for ups_config in ups_configs.iter() {
        ...
        if config._port.is_some() {
            for port_server_config in config._port.as_ref().unwrap()._server.iter() {
                ...
                let ups_config = config_toml::UpstreamConfig {
                    name: upstream_name,
                    server: vec![port_server_config.proxy_pass.clone()],
                };
                ups_configs.push(ups_config);
            }
        }
        ...
        if config._domain.is_some() {
            for domain_server_config in config._domain.as_ref().unwrap()._server.iter() {
                ...
                let ups_config = config_toml::UpstreamConfig {
                    ...
                    server: vec![domain_server_config.proxy_pass.clone()],
                };
                ups_configs.push(ups_config);
            }
        }
        ...
        for ups_config in ups_configs.iter() {
            ...
            for proxy_pass in ups_config.server.iter() {
            ...
            let ups_data = Rc::new(RefCell::new(Some(UpstreamData {
                is_disable_change: false,
                is_change: false,
    ...
    pub fn heartbeat(
        ...
        let (heartbeat, connect): (Option<UpstreamHeartbeat>, Rc<Box<dyn connect::Connect>>) =
            match &proxy_pass {
                config_toml::ProxyPass::Tcp(tcp) => {
