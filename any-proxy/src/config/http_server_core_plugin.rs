...
pub type PluginHandleProtocol =
    fn(ServerArg, BufReader<StreamFlow>) -> Pin<Box<dyn Future<Output = Result<()>> + Send>>;
...
impl Conf {
    pub fn new() -> Self {
        Conf {
            plugin_handle_protocol: ArcRwLockTokio::default(),
        }
    }
}
...
lazy_static! {
    pub static ref M: typ::ArcRwLock<module::Module> = typ::ArcRwLock::new(module::Module {
        name: "http_server_core_plugin".to_string(),
        main_index: -1,
        ctx_index: -1,
        index: -1,
        ctx_index_len: -1,
