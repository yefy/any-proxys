...
lazy_static! {
    pub static ref MODULE_CMDS: Arc<Vec<module::Cmd>> = Arc::new(vec![module::Cmd {
        name: "net_server_http_purge".to_string(),
        set: |ms, conf_arg, cmd, conf| Box::pin(net_server_http_purge(ms, conf_arg, cmd, conf)),
...
async fn init_conf(
    ...
    use crate::proxy::http_proxy::http_server_purge;
    let net_core_plugin_conf = net_core_plugin::main_conf_mut(&ms).await;
    net_core_plugin_conf
        .plugin_handle_http
        .get_mut()
        .await
        .push(|r| Box::pin(http_server_purge::server_handle(r)));
    return Ok(());
}

async fn net_server_http_purge(
    _ms: module::Modules,
