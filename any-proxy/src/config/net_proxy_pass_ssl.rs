...
lazy_static! {
    pub static ref M: typ::ArcRwLock<module::Module> = typ::ArcRwLock::new(module::Module {
        name: "net_proxy_pass_ssl".to_string(),
        main_index: -1,
        ctx_index: -1,
        index: -1,
        ctx_index_len: -1,
...
async fn proxy_pass_ssl(
    ...
    if address_vars.is_var {
        ...
        net_server_core_conf.upstream_name = "upstream_var_ssl".to_string();
    ...
    use super::upstream_proxy_pass_ssl;
    let heartbeat: Arc<Box<dyn upstream_core::HeartbeatI>> = Arc::new(Box::new(
        upstream_proxy_pass_ssl::Heartbeat::new(proxy_pass_conf.clone()),
    ));
