...
use crate::proxy::http_proxy::http_filter::http_filter_header_parse::{
    http_filter_header_parse, set_header_filter,
};
...
lazy_static! {
    pub static ref M: typ::ArcRwLock<module::Module> = typ::ArcRwLock::new(module::Module {
        name: "http_filter_header_parse".to_string(),
        main_index: -1,
        ctx_index: -1,
        index: -1,
        ctx_index_len: -1,
...
async fn init_conf(
...

    {
        ...
        (plugin_http_header_filter_set)(|r| Box::pin(http_filter_header_parse(r))).await?;
    }
