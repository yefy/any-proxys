use crate::proxy::http_proxy::http_filter::http_filter_headers::{
    http_filter_headers, set_header_filter,
};
...
lazy_static! {
    pub static ref MODULE_CMDS: Arc<Vec<module::Cmd>> = Arc::new(vec![module::Cmd {
        name: "wasm_http_filter_headers".to_string(),
        set: |ms, conf_arg, cmd, conf| Box::pin(wasm_http_filter_headers(ms, conf_arg, cmd, conf)),
...
lazy_static! {
    pub static ref M: typ::ArcRwLock<module::Module> = typ::ArcRwLock::new(module::Module {
        name: "http_filter_headers".to_string(),
        main_index: -1,
        ctx_index: -1,
        index: -1,
        ctx_index_len: -1,
...
async fn init_conf(
...

    {
        ...
        (plugin_http_header_filter_set)(|r| Box::pin(http_filter_headers(r))).await?;
    }
...
async fn wasm_http_filter_headers(
    _ms: module::Modules,
