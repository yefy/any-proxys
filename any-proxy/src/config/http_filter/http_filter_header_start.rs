...
lazy_static! {
    pub static ref M: typ::ArcRwLock<module::Module> = typ::ArcRwLock::new(module::Module {
        name: "http_filter_header_start".to_string(),
        main_index: -1,
        ctx_index: -1,
        index: -1,
        ctx_index_len: -1,
...
async fn init_conf(
    ...
    let (plugin_http_header_filter, plugin_http_header_filter_set) = {
        let net_core_plugin_conf = net_core_plugin::main_conf_mut(&ms).await;
        (
            net_core_plugin_conf.plugin_http_header_filter.clone(),
            net_core_plugin_conf.plugin_http_header_filter_set.clone(),
        )
    };

    if plugin_http_header_filter.is_none().await {
        plugin_http_header_filter
            .set(|r| Box::pin(http_filter_header_start(r)))
            .await;
    }
    if plugin_http_header_filter_set.is_none().await {
