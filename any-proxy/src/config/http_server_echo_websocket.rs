...
lazy_static! {
    pub static ref MODULE_CMDS: Arc<Vec<module::Cmd>> = Arc::new(vec![module::Cmd {
        name: "http_server_echo_websocket".to_string(),
        set: |ms, conf_arg, cmd, conf| Box::pin(http_server_echo_websocket(
...
async fn http_server_echo_websocket(
    _ms: module::Modules,
    ...
    log::trace!("http_server_echo_websocket echo_conf:{:?}", echo_conf);
    ...
    use crate::proxy::websocket_proxy::websocket_echo_server;
    http_server_core_plugin_conf.plugin_handle_protocol = ArcRwLockTokio::new(|arg, flow| {
        Box::pin(websocket_echo_server::http_server_handle(arg, flow))
    });
