...
lazy_static! {
    pub static ref MODULE_CMDS: Arc<Vec<module::Cmd>> = Arc::new(vec![module::Cmd {
        name: "http_server_echo".to_string(),
        set: |ms, conf_arg, cmd, conf| Box::pin(http_server_echo(ms, conf_arg, cmd, conf)),
...
async fn http_server_echo(
    ...
    log::trace!("http_server_echo echo_conf:{:?}", echo_conf);
