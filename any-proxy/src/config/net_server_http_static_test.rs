...
pub struct Conf {
    pub conf: Option<HttpServerStaticConfig>,
}
...
lazy_static! {
    pub static ref MODULE_CMDS: Arc<Vec<module::Cmd>> = Arc::new(vec![module::Cmd {
        name: "net_server_http_static_test".to_string(),
        set: |ms, conf_arg, cmd, conf| Box::pin(net_server_http_static_test(
...
async fn init_conf(
    ...
    use crate::proxy::http_proxy::http_server_static_test;
    let net_core_plugin_conf = net_core_plugin::main_conf_mut(&ms).await;
    net_core_plugin_conf
        .plugin_handle_http
        .get_mut()
        .await
        .push(|r| Box::pin(http_server_static_test::server_handle(r)));
    return Ok(());
}

async fn net_server_http_static_test(
    _ms: module::Modules,
    ...
    log::trace!(target: "main", "net_server_http_static_test static_conf:{:?}", static_conf);
    conf.conf = Some(static_conf);
