...
lazy_static! {
    pub static ref MODULE_CMDS: Arc<Vec<module::Cmd>> = Arc::new(vec![module::Cmd {
        name: "net_server_websocket".to_string(),
        set: |ms, conf_arg, cmd, conf| Box::pin(net_server_websocket(ms, conf_arg, cmd, conf)),
...
async fn net_server_websocket(
    ...
    log::trace!(target: "main", "net_server_websocket str:{:?}", str);
    conf.body = Some(str);
    ...
    use crate::proxy::websocket_proxy;
    net_server_core_plugin_conf.plugin_handle_protocol =
        ArcRwLockTokio::new(|arg, flow| Box::pin(websocket_proxy::server_handle(arg, flow)));
