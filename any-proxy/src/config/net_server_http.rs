...
lazy_static! {
    pub static ref MODULE_CMDS: Arc<Vec<module::Cmd>> = Arc::new(vec![module::Cmd {
        name: "net_server_http".to_string(),
        set: |ms, conf_arg, cmd, conf| Box::pin(net_server_http(ms, conf_arg, cmd, conf)),
...
async fn merge_conf(
    ...
    if parent_conf.is_some() {
        ...
        if child_conf.data.is_none() {
            child_conf.data = parent_conf.data.clone();
        }
    }
...
async fn net_server_http(
    ...
    log::trace!(target: "main", "net_server_http str:{:?}", str);
    conf.data = Some(str);
    ...
    net_server_core_plugin_conf.plugin_handle_protocol =
        ArcRwLockTokio::new(|arg, flow| Box::pin(http_proxy::http_server_handle(arg, flow)));
    return Ok(());
}
