...
lazy_static! {
    pub static ref M: typ::ArcRwLock<module::Module> = typ::ArcRwLock::new(module::Module {
        name: "net_proxy_pass_tunnel2".to_string(),
        main_index: -1,
        ctx_index: -1,
        index: -1,
        ctx_index_len: -1,
...
pub async fn do_proxy_pass_tunnel2_tcp(
    ...
    conf_arg: &module::ConfArg,
    _cmd: &module::Cmd,
    mut proxy_pass_conf: ProxyPassTcpTunnel2,
) -> Result<()> {
    let str = conf_arg.value.get::<String>();
    ...
    use super::upstream_proxy_pass_tunnel2;
    let heartbeat: Arc<Box<dyn upstream_core::HeartbeatI>> = Arc::new(Box::new(
        upstream_proxy_pass_tunnel2::HeartbeatTcp::new(proxy_pass_conf.clone()),
    ));
...
pub async fn do_proxy_pass_tunnel2_ssl(
    ...
    conf_arg: &module::ConfArg,
    _cmd: &module::Cmd,
    mut proxy_pass_conf: ProxyPassSslTunnel2,
) -> Result<()> {
    let str = conf_arg.value.get::<String>();
    if proxy_pass_conf.balancer.is_empty() {
        proxy_pass_conf.balancer = upstream_core_plugin::ROUND_ROBIN.to_string();
    ...
    use super::upstream_proxy_pass_tunnel2;
    let heartbeat: Arc<Box<dyn upstream_core::HeartbeatI>> = Arc::new(Box::new(
        upstream_proxy_pass_tunnel2::HeartbeatSsl::new(proxy_pass_conf.clone()),
    ));
...
pub async fn do_proxy_pass_tunnel2_quic(
    ...
    conf_arg: &module::ConfArg,
    _cmd: &module::Cmd,
    mut proxy_pass_conf: ProxyPassQuicTunnel2,
) -> Result<()> {
    let str = conf_arg.value.get::<String>();
    if proxy_pass_conf.balancer.is_empty() {
        proxy_pass_conf.balancer = upstream_core_plugin::ROUND_ROBIN.to_string();
    ...
    use super::upstream_proxy_pass_tunnel2;
    let heartbeat: Arc<Box<dyn upstream_core::HeartbeatI>> = Arc::new(Box::new(
        upstream_proxy_pass_tunnel2::HeartbeatQuic::new(proxy_pass_conf.clone()),
    ));
