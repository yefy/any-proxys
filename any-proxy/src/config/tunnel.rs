...
lazy_static! {
    pub static ref MODULE_CMDS: Arc<Vec<module::Cmd>> = Arc::new(vec![module::Cmd {
        ...
        set: |ms, conf_arg, cmd, conf| Box::pin(tunnel(ms, conf_arg, cmd, conf)),
...
async fn init_master_thread_confs(
    ...
    for module in ms.get_modules() {
        ...
        log::trace!(target: "main",
                    "tunnel init_master_thread name:{}, typ:{}, main_index:{}, ctx_index:{}",
                    name,
                    typ,
                    main_index,
                    ctx_index
...
async fn init_work_thread_confs(
    ...
    for module in ms.get_modules() {
        ...
        log::trace!(target: "main",
                    "tunnel init_work_thread name:{}, typ:{}, main_index:{}, ctx_index:{}",
                    name,
                    typ,
                    main_index,
                    ctx_index
...
async fn tunnel(
    ...
    conf_arg
        .module_type
        .store(conf::MODULE_TYPE_TUNNEL, Ordering::SeqCst);
    conf_arg
        .cmd_conf_type
