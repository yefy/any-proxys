...
use crate::proxy::stream_stream_tmp_file::{
    cache_handle_run, memory_handle_run, set_cache_handle, set_memory_handle,
};
...
lazy_static! {
    pub static ref M: typ::ArcRwLock<module::Module> = typ::ArcRwLock::new(module::Module {
        name: "stream_stream_tmp_file".to_string(),
        main_index: -1,
        ctx_index: -1,
        index: -1,
        ctx_index_len: -1,
...
async fn init_conf(
    ...
    let (plugin_handle_set_stream_cache, plugin_handle_set_stream_memory) = {
        let http_core_plugin_conf = http_core_plugin::main_conf_mut(&ms).await;
        (
            http_core_plugin_conf.plugin_handle_set_stream_cache.clone(),
    ...
    {
        let plugin_handle_set_stream_cache = plugin_handle_set_stream_cache.get_mut().await;
        (plugin_handle_set_stream_cache)(|sss| Box::pin(cache_handle_run(sss))).await?;
    }

    plugin_handle_set_stream_cache
        .set(|plugin| Box::pin(set_cache_handle(plugin)))
        .await;

    {
        let plugin_handle_set_stream_memory = plugin_handle_set_stream_memory.get_mut().await;
        (plugin_handle_set_stream_memory)(|sss| Box::pin(memory_handle_run(sss))).await?;
    }

    plugin_handle_set_stream_memory
        .set(|plugin| Box::pin(set_memory_handle(plugin)))
        .await;
    return Ok(());
}
