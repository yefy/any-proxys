...
lazy_static! {
    pub static ref MODULE_CMDS: Arc<Vec<module::Cmd>> = Arc::new(vec![module::Cmd {
        name: "http_server_static_websocket".to_string(),
        set: |ms, conf_arg, cmd, conf| Box::pin(http_server_static_websocket(
...
async fn http_server_static_websocket(
    _ms: module::Modules,
    ...
    log::trace!("http_server_static static_conf:{:?}", static_conf);
