...
lazy_static! {
    pub static ref MODULE_CMDS: Arc<Vec<module::Cmd>> = Arc::new(vec![module::Cmd {
        name: "net_server_websocket_proxy".to_string(),
        set: |ms, conf_arg, cmd, conf| Box::pin(net_server_websocket_proxy(
...
async fn init_conf(
    ...
    use crate::proxy::websocket_proxy::websocket_server_proxy;
    ...
    net_core_plugin_conf
        ...
        .push(|arg, client_stream| {
            Box::pin(websocket_server_proxy::server_handle(arg, client_stream))
        });
    return Ok(());
}

async fn net_server_websocket_proxy(
    _ms: module::Modules,
    ...
    log::trace!(target: "main", "net_server_websocket_proxy str:{:?}", str);
    conf.body = Some(str);
