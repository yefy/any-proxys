...
async fn server(
    ...
    let net_confs = {
        let net_confs = conf.get_mut::<Vec<typ::ArcUnsafeAny>>();
        net_confs.clone()
    };
    ...
    for module in ms.get_modules() {
        ...
        server_confs.push(conf);
    }

    {
        let ctx_index = net::module().get().ctx_index;
        let net_conf = net_confs[ctx_index as usize].clone();
        let net_conf_ = net_conf.get_mut::<net::Conf>();

        let ctx_index = module().get().ctx_index;
        let server_conf = server_confs[ctx_index as usize].clone();
        let server_conf_ = server_conf.get_mut::<Conf>();
        server_conf_.net_confs = net_confs.clone();
        server_conf_.server_confs = server_confs.clone();

        net_conf_.sub_confs.push(server_conf.clone());
    }

    let server_confs_ = typ::ArcUnsafeAny::new(Box::new(server_confs.clone()));
    conf_arg.module_type = Arc::new(AtomicUsize::new(conf::MODULE_TYPE_NET));
    ...
    // conf_arg_sub
    //     .cmd_conf_type
    //     .store(conf::CMD_CONF_TYPE_SERVER, Ordering::SeqCst);
    // conf_arg_sub
    //     .main_index
    ...
    // conf_arg_sub.parent_module_confs = net_confs;
    ...
    ms.parse_config(&mut conf_arg, server_confs_).await?;
