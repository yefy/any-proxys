...
lazy_static! {
    pub static ref MODULE_CMDS: Arc<Vec<module::Cmd>> = Arc::new(vec![module::Cmd {
        name: "net_server_static_websocket".to_string(),
        set: |ms, conf_arg, cmd, conf| Box::pin(net_server_static_websocket(
...
async fn net_server_static_websocket(
    _ms: module::Modules,
    ...
    log::trace!(target: "main", "net_server_static_http static_conf:{:?}", static_conf);
    ...
    use crate::proxy::websocket_proxy::websocket_static_server;
    net_server_core_plugin_conf.plugin_handle_protocol = ArcRwLockTokio::new(|arg, flow| {
        Box::pin(websocket_static_server::http_server_handle(arg, flow))
    });
