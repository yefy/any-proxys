...
use crate::proxy::stream_stream_cache::{cache_handle_run, set_cache_handle};
...
lazy_static! {
    pub static ref M: typ::ArcRwLock<module::Module> = typ::ArcRwLock::new(module::Module {
        name: "stream_stream_cache".to_string(),
        main_index: -1,
        ctx_index: -1,
        index: -1,
        ctx_index_len: -1,
...
async fn init_conf(
    ...
    let (plugin_handle_stream_cache, plugin_handle_set_stream_cache) = {
    ...
    if plugin_handle_stream_cache.is_none().await {
        plugin_handle_stream_cache
            .set(|sss| Box::pin(cache_handle_run(sss)))
            .await;
    }
    if plugin_handle_set_stream_cache.is_none().await {
