...
lazy_static! {
    pub static ref MODULE_CMDS: Arc<Vec<module::Cmd>> = Arc::new(vec![module::Cmd {
        name: "net_server_websocket_static".to_string(),
        set: |ms, conf_arg, cmd, conf| Box::pin(net_server_websocket_static(
...
async fn init_conf(
    ...
    use crate::proxy::websocket_proxy::websocket_server_static;
    ...
    net_core_plugin_conf
        ...
        .push(|arg, client_stream| {
            Box::pin(websocket_server_static::server_handle(arg, client_stream))
        });
    return Ok(());
}

async fn net_server_websocket_static(
    _ms: module::Modules,
    ...
    log::trace!(target: "main", "net_server_http_static static_conf:{:?}", static_conf);
