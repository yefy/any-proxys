...
lazy_static! {
    pub static ref MODULE_CMDS: Arc<Vec<module::Cmd>> = Arc::new(vec![module::Cmd {
        ...
        set: |ms, conf_arg, cmd, conf| Box::pin(common(ms, conf_arg, cmd, conf)),
...
async fn create_main_confs(mut ms: module::Modules) -> Result<ArcUnsafeAny> {
    ...
    for module in module::get_modules().get().get_modules() {
        ...
        if typ & conf::MODULE_TYPE_COMMON == 0 {
            continue;
        }
...
async fn merge_old_main_confs(
    ...
    for module in ms.get_modules() {
        ...
        log::trace!(target: "main",
            "common merge_old_main_confs name:{}, typ:{}, main_index:{}, ctx_index:{}",
            name,
            typ,
            main_index,
            ctx_index
...
async fn init_main_confs(ms: module::Modules, conf: typ::ArcUnsafeAny) -> Result<()> {
    ...
    for module in ms.get_modules() {
        ...
        log::trace!(target: "main",
            "common init_main_confs name:{}, typ:{}, main_index:{}, ctx_index:{}",
            name,
            typ,
            main_index,
            ctx_index
...
async fn init_master_thread_confs(
    ...
    for module in ms.get_modules() {
        ...
        log::trace!(target: "main",
                    "common init_master_thread name:{}, typ:{}, main_index:{}, ctx_index:{}",
                    name,
                    typ,
                    main_index,
                    ctx_index
...
async fn init_work_thread_confs(
    ...
    for module in ms.get_modules() {
        ...
        log::trace!(target: "main",
                    "common init_work_thread name:{}, typ:{}, main_index:{}, ctx_index:{}",
                    name,
                    typ,
                    main_index,
                    ctx_index
...
async fn common(
    ...
    conf_arg
        .module_type
        .store(conf::MODULE_TYPE_COMMON, Ordering::SeqCst);
    conf_arg
        .cmd_conf_type
