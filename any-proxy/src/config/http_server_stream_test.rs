...
lazy_static! {
    pub static ref MODULE_CMDS: Arc<Vec<module::Cmd>> = Arc::new(vec![module::Cmd {
        name: "http_server_stream_test".to_string(),
        set: |ms, conf_arg, cmd, conf| Box::pin(http_server_stream_test(ms, conf_arg, cmd, conf)),
...
async fn http_server_stream_test(
    _ms: module::Modules,
    ...
    log::trace!("http_server_stream_test:{:?}", config);
    conf.config = config;
