...
use crate::proxy::stream_stream_memory::{memory_handle_run, set_memory_handle};
...
lazy_static! {
    pub static ref M: typ::ArcRwLock<module::Module> = typ::ArcRwLock::new(module::Module {
        name: "stream_stream_memory".to_string(),
        main_index: -1,
        ctx_index: -1,
        index: -1,
        ctx_index_len: -1,
...
async fn init_conf(
    ...
    if plugin_handle_stream_memory.is_none().await {
        plugin_handle_stream_memory
            .set(|sss| Box::pin(memory_handle_run(sss)))
            .await;
    }

    if plugin_handle_set_stream_memory.is_none().await {
