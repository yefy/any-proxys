...
lazy_static! {
    pub static ref MODULE_CMDS: Arc<Vec<module::Cmd>> = Arc::new(vec![module::Cmd {
        name: "port_listen_tcp".to_string(),
        set: |ms, conf_arg, cmd, conf| Box::pin(port_listen_tcp(ms, conf_arg, cmd, conf)),
...
async fn port_listen_tcp(
    ...
    for addr in sock_addrs.iter() {
        ...
        let listen_server: Arc<Box<dyn server::Server>> =
            Arc::new(Box::new(tcp_server::Server::new(
                addr.clone(),
