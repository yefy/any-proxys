...
lazy_static! {
    pub static ref MODULE_CMDS: Arc<Vec<module::Cmd>> = Arc::new(vec![module::Cmd {
        name: "port_listen_quic".to_string(),
        set: |ms, conf_arg, cmd, conf| Box::pin(port_listen_quic(ms, conf_arg, cmd, conf)),
...
async fn port_listen_quic(
    ...
    for addr in sock_addrs.iter() {
        let key = util::util::udp_key_from_addr(addr)
            .map_err(|e| anyhow!("err:PortConfig::udp_key_from_addr => e:{}", e))?;
        if port_core_conf.port_config_listen_map.get(&key).is_some() {
            return Err(anyhow!("err:addr is exist => addr:{}", addr));
        }

        let quic_config = socket_quic_conf.config(&net_core_conf.quic_config_name);
        if quic_config.is_none() {
            return Err(anyhow!(
                "err:quic_config_name => quic_config_name:{}",
                net_core_conf.quic_config_name
            ));
        }
        ...
        let listen = config_toml::Listen {
            ...
            ssl: Some(quic_listen.ssl.clone()),
        };
