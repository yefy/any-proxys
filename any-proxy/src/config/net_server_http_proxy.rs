...
lazy_static! {
    pub static ref MODULE_CMDS: Arc<Vec<module::Cmd>> = Arc::new(vec![module::Cmd {
        name: "net_server_http_proxy".to_string(),
        set: |ms, conf_arg, cmd, conf| Box::pin(net_server_http_proxy(ms, conf_arg, cmd, conf)),
...
async fn init_conf(
    ...
    net_core_plugin_conf
        ...
        .get_mut()
        .await
        .push(|r| Box::pin(http_server_proxy::server_handle(r)));
    return Ok(());
}

async fn net_server_http_proxy(
    _ms: module::Modules,
    ...
    log::trace!(target: "main", "net_server_http_proxy proxy_conf:{:?}", proxy_conf);
