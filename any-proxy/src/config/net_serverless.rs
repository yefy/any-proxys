...
lazy_static! {
    pub static ref MODULE_CMDS: Arc<Vec<module::Cmd>> = Arc::new(vec![module::Cmd {
        name: "wasm_serverless".to_string(),
        set: |ms, conf_arg, cmd, conf| Box::pin(wasm_serverless(ms, conf_arg, cmd, conf)),
...
async fn init_conf(
    ...
    net_core_plugin_conf
        .plugin_handle_serverless
        .get_mut()
        .await
        .push(|stream_info| Box::pin(serverless(stream_info.clone())));

    net_core_plugin_conf
        .is_plugin_handle_serverless
        .get_mut()
        .await
        .push(|stream_info| Box::pin(is_serverless(stream_info)));

    return Ok(());
}

async fn wasm_serverless(
    _ms: module::Modules,
...
pub async fn is_serverless(stream_info: Share<StreamInfo>) -> Result<bool> {
    ...
    for wasm_plugin_conf in &conf.wasm_plugin_confs.as_ref().unwrap().wasm {
        if wasm_plugin_conf.is_open {
            return Ok(true);
        }
    }
    return Ok(true);
...
pub async fn do_wasm_serverless(stream_info: Share<StreamInfo>) -> Result<crate::Error> {
    ...
    for wasm_plugin_conf in &wasm_plugin_confs.wasm {
        ...
        match ret.unwrap() {
            ...
            crate::Error::Error => {
                return Err(anyhow::anyhow!("err:wasm_serverless"));
            }
            crate::Error::Return => {
