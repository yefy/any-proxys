...
lazy_static! {
    pub static ref MODULE_CMDS: Arc<Vec<module::Cmd>> = Arc::new(vec![module::Cmd {
        name: "net_server_echo_http".to_string(),
        set: |ms, conf_arg, cmd, conf| Box::pin(net_server_echo_http(ms, conf_arg, cmd, conf)),
...
async fn net_server_echo_http(
    ...
    log::trace!(target: "main", "net_server_echo_http echo_conf:{:?}", echo_conf);
    ...
    use crate::proxy::http_proxy::http_echo_server;
    net_server_core_plugin_conf.plugin_handle_protocol =
        ArcRwLockTokio::new(|arg, flow| Box::pin(http_echo_server::http_server_handle(arg, flow)));
    return Ok(());
}
