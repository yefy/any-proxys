...
pub struct Conf {
    pub proxy: HttpServerProxyConfig,
}

impl Conf {
    pub fn new() -> Self {
        Conf {
            proxy: HttpServerProxyConfig {
...
lazy_static! {
    pub static ref MODULE_CMDS: Arc<Vec<module::Cmd>> = Arc::new(vec![module::Cmd {
        name: "http_server_proxy".to_string(),
        set: |ms, conf_arg, cmd, conf| Box::pin(http_server_proxy(ms, conf_arg, cmd, conf)),
...
async fn http_server_proxy(
    ...
    log::trace!("http_server_proxy proxy_conf:{:?}", proxy_conf);
    conf.proxy = proxy_conf;
    ...
    use crate::proxy::http_proxy::http_server;
    http_server_core_plugin_conf.plugin_handle_protocol =
        ArcRwLockTokio::new(|arg, flow| Box::pin(http_server::http_server_handle(arg, flow)));
    return Ok(());
}
