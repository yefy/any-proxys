...
lazy_static! {
    pub static ref M: typ::ArcRwLock<module::Module> = typ::ArcRwLock::new(module::Module {
        name: "http_in_header_start".to_string(),
        main_index: -1,
        ctx_index: -1,
        index: -1,
        ctx_index_len: -1,
...
async fn init_conf(
    ...
    let (plugin_http_header_in, plugin_http_header_in_set) = {
        let net_core_plugin_conf = net_core_plugin::main_conf_mut(&ms).await;
        (
            net_core_plugin_conf.plugin_http_header_in.clone(),
            net_core_plugin_conf.plugin_http_header_in_set.clone(),
        )
    };

    if plugin_http_header_in.is_none().await {
        plugin_http_header_in
            .set(|r_in| Box::pin(http_in_header_start(r_in)))
            .await;
    }
    if plugin_http_header_in_set.is_none().await {
...
pub async fn http_in_header_start(r: Arc<HttpStreamRequest>) -> Result<()> {
    log::trace!(target: "main", "r.session_id:{}, http_in_header_start", r.session_id);
