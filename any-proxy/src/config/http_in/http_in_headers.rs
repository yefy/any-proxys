use crate::proxy::http_proxy::http_in::http_in_headers::{http_in_headers, set_header_filter};
...
lazy_static! {
    pub static ref MODULE_CMDS: Arc<Vec<module::Cmd>> = Arc::new(vec![module::Cmd {
        name: "wasm_http_in_headers".to_string(),
        set: |ms, conf_arg, cmd, conf| Box::pin(wasm_http_in_headers(ms, conf_arg, cmd, conf)),
...
lazy_static! {
    pub static ref M: typ::ArcRwLock<module::Module> = typ::ArcRwLock::new(module::Module {
        name: "http_in_headers".to_string(),
        main_index: -1,
        ctx_index: -1,
        index: -1,
        ctx_index_len: -1,
...
async fn init_conf(
    ...
    let plugin_http_header_in_set = {
        let net_core_plugin_conf = net_core_plugin::main_conf_mut(&ms).await;
        net_core_plugin_conf.plugin_http_header_in_set.clone()
    };

    {
        let plugin_http_header_in_set = plugin_http_header_in_set.get_mut().await;
        (plugin_http_header_in_set)(|r_in| Box::pin(http_in_headers(r_in))).await?;
    }

    plugin_http_header_in_set
