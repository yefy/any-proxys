...
lazy_static! {
    pub static ref MODULE_CMDS: Arc<Vec<module::Cmd>> = Arc::new(vec![module::Cmd {
        name: "net_server_echo_websocket".to_string(),
        set: |ms, conf_arg, cmd, conf| Box::pin(net_server_echo_websocket(ms, conf_arg, cmd, conf)),
...
async fn net_server_echo_websocket(
    _ms: module::Modules,
