...
lazy_static! {
    pub static ref M: typ::ArcRwLock<module::Module> = typ::ArcRwLock::new(module::Module {
        name: "net_proxy_pass_quic".to_string(),
        main_index: -1,
        ctx_index: -1,
        index: -1,
        ctx_index_len: -1,
...
pub async fn do_proxy_pass_quic_var(
    proxy_pass_conf: ProxyPassQuic,
...
pub async fn do_proxy_pass_quic(
    ms: &module::Modules,
    conf_arg: &module::ConfArg,
    _cmd: &module::Cmd,
    mut proxy_pass_conf: ProxyPassQuic,
) -> Result<()> {
    let str = conf_arg.value.get::<String>();
    ...
    use super::upstream_proxy_pass_quic;
    let heartbeat: Arc<Box<dyn upstream_core::HeartbeatI>> = Arc::new(Box::new(
        upstream_proxy_pass_quic::Heartbeat::new(proxy_pass_conf.clone()),
    ));
