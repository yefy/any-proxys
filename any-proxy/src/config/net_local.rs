...
impl Drop for Conf {
    fn drop(&mut self) {
        log::debug!(target: "ms", "drop net_core");
    }
}
...
async fn local(
    ...
    {
        let ctx_index = net_server::module().get().ctx_index;
        let server_conf = server_confs[ctx_index as usize].clone();
        let server_conf_ = server_conf.get_mut::<net_server::Conf>();
        ...
        local_conf_.net_confs = server_conf_.net_confs.clone();
