...
lazy_static! {
    pub static ref MODULE_CMDS: Arc<Vec<module::Cmd>> = Arc::new(vec![module::Cmd {
        name: "net_server_websocket_echo".to_string(),
        set: |ms, conf_arg, cmd, conf| Box::pin(net_server_websocket_echo(ms, conf_arg, cmd, conf)),
...
async fn init_conf(
    ...
    use crate::proxy::websocket_proxy::websocket_server_echo;
    let net_core_plugin_conf = net_core_plugin::main_conf_mut(&ms).await;
    net_core_plugin_conf
        .plugin_handle_websocket
        .get_mut()
        .await
        .push(|arg, client_stream| {
            Box::pin(websocket_server_echo::server_handle(arg, client_stream))
        });
    return Ok(());
}

async fn net_server_websocket_echo(
    _ms: module::Modules,
    ...
    log::trace!(target: "main", "net_server_websocket_echo echo_conf:{:?}", echo_conf);
    conf.body = Some(echo_conf.body);

    use crate::config::net_server_websocket;
    let net_server_websocket_conf = net_server_websocket::curr_conf_mut(conf_arg.last_conf());
    if net_server_websocket_conf.body.is_none() {
        return Err(anyhow!("err:net_server_websocket nil"));
    }
