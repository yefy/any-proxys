...
lazy_static! {
    pub static ref MODULE_CMDS: Arc<Vec<module::Cmd>> = Arc::new(vec![module::Cmd {
        name: "domain_listen_tcp".to_string(),
        set: |ms, conf_arg, cmd, conf| Box::pin(domain_listen_tcp(ms, conf_arg, cmd, conf)),
...
async fn domain_listen_tcp(
    ...
    log::trace!(target: "main", "domain_listen_tcp tcp_listen:{:?}", tcp_listen);
