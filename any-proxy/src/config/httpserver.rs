...
async fn server(
    ...
    let http_confs = {
        let http_confs = conf.get_mut::<Vec<typ::ArcUnsafeAny>>();
        http_confs.clone()
    };
    ...
    {
        let ctx_index = http::module().get().ctx_index;
        let http_conf = http_confs[ctx_index as usize].clone();
        let http_conf_ = http_conf.get_mut::<http::Conf>();
        ...
        server_conf_.http_confs = http_confs.clone();
        server_conf_.server_confs = server_confs.clone();

        http_conf_.sub_confs.push(server_conf.clone());
    }
    ...
    conf_arg_sub.parent_module_confs = http_confs;

    ms.parse_config(&mut conf_arg_sub, server_confs_).await?;
