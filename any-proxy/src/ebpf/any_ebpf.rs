...
impl EbpfGroup {
    pub fn new(cpu_affinity: bool, group_version: i32) -> Result<EbpfGroup> {
        let thread_spawn = ThreadSpawn::new(cpu_affinity, group_version);
        Ok(EbpfGroup { thread_spawn })
    }
...
impl EbpfSockhash {
    ...
    pub async fn add_sockhash(
        ...
        log::info!("socket_data:{:?}", socket_data);
        ...
        if socket_data.ups_fd > 0 {
            ...
            log::info!("ups_fd:{}, key_bytes:{:?}", socket_data.ups_fd, key_bytes);
            maps_mut
                .sk_index_hash()
                .update(
                    key_bytes,
    ...
    pub async fn del_sockhash(
        ...
        log::info!("del socket_data:{:?}", socket_data);
    ...
    pub async fn add_reuseport(
        ...
        log::info!("reuseport_data:{:?}", reuseport_data);
        let mut maps_mut = skel.maps_mut();
        maps_mut
            .quic_sockhash()
            .update(
