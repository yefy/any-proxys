...
impl<T: 'static> Future for BlockingHandle<T> {
    ...
    fn poll(self: Pin<&mut Self>, _cx: &mut Context<'_>) -> Poll<Self::Output> {
        match &mut self.get_mut().0 {
            #[cfg(any(feature = "tokio_tp", feature = "tokio_ct"))]
            //
            InnerBh::Tokio(handle) => match ready!(Pin::new(handle).poll(_cx)) {
                Ok(t) => Poll::Ready(t),

                Err(e) => panic!(
                    "Task has been canceled or has panicked. \
						Are you dropping the executor to early? Error: {}",
                    e
                ),
            },

            #[cfg(feature = "async_std")]
            InnerBh::AsyncStd(handle) => Pin::new(handle).poll(_cx),
            #[cfg(feature = "async_global")]
            InnerBh::AsyncGlobal(task) => Pin::new(task).poll(_cx),
