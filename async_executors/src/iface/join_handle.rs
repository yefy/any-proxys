...
impl<T> JoinHandle<T> {
    ...
    pub fn remote_handle(handle: RemoteHandle<T>) -> Self {
        let inner = InnerJh::RemoteHandle {
            handle: Some(handle),
        };
...
enum InnerJh<T> {
    ...
    #[cfg(feature = "async_global")]
    //
    AsyncGlobal { task: Option<AsyncGlobalTask<T>> },
    ...
    //
    RemoteHandle { handle: Option<RemoteHandle<T>> },
}
...
impl<T: 'static> Future for JoinHandle<T> {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        match &mut self.get_mut().inner {
            #[cfg(any(feature = "tokio_tp", feature = "tokio_ct"))]
            //
            InnerJh::Tokio { handle, .. } => match ready!(Pin::new(handle).poll(cx)) {
                Ok(t) => Poll::Ready(t),
            ...
            #[cfg(feature = "async_std")]
            InnerJh::AsyncStd { handle, .. } => match ready!(Pin::new(handle).poll(cx)) {
                Ok(t) => Poll::Ready(t),
                Err(_) => unreachable!(),
            },

            #[cfg(feature = "async_global")]
            InnerJh::AsyncGlobal { task, .. } => Pin::new(task.as_mut().unwrap()).poll(cx),

            InnerJh::RemoteHandle { ref mut handle } => Pin::new(handle)
                .as_pin_mut()
                .expect("no polling after detach")
                .poll(cx),
        }
    }
}
