...
where
    T: StreamReadWriteFlow + Unpin,
    B: Buf,
{
    pub(crate) fn new(io: T) -> Buffered<T, B> {
        let write_strategy = if io.is_write_vectored() {
            WriteStrategy::Queue
        } else {
            WriteStrategy::Flatten
        };

        let (strategy, is_sendfile) = if io.raw_fd() > 0 {
            (WriteStrategy::Queue, true)
        } else {
            if io.is_write_vectored() {
                (WriteStrategy::Queue, false)
            } else {
                (WriteStrategy::Flatten, false)
            }
        };

        let write_buf = WriteBuf::new(strategy, write_strategy, is_sendfile);
    ...
    pub(crate) fn poll_flush(&mut self, cx: &mut task::Context<'_>) -> Poll<io::Result<()>> {
        if self.flush_pipeline && !self.read_buf.is_empty() {
            ...
            if self.write_buf.is_sendfile() {
                if let WriteStrategy::Flatten = self.write_buf.write_strategy {
                    ready!(self.poll_flush_flattened_sendfile(cx))?;
                    loop {
                        self.write_buf.queue_to_cache();
                        ready!(self.poll_flush_flattened_cache(cx))?;

                        if !self.buf_file.has_remaining() {
                            let buf = self.write_buf.queue.front_mut();
                            if buf.is_none() {
                                return Pin::new(&mut self.io).poll_flush(cx);
                            }

                            let buf: &mut B = &mut buf.unwrap();

                            let buf_file = MsgReadBufFile::from_slice(buf.chunk());
                            if buf_file.is_none() {
                                continue;
                            }
                            self.buf_file = buf_file.unwrap();
                        }

                        #[cfg(unix)]
                        if !self.is_set_tcp_nopush {
                            self.is_set_tcp_nopush = true;
                            use any_base::util::set_tcp_nopush_;
                            if self.io.raw_fd() > 0 {
                                set_tcp_nopush_(self.io.raw_fd(), true);
                            }
                        }

                        if self.io.write_cache_size() > 0 {
                ...
                loop {
                    let (n, len) = {
                        ...
                        let len = self.write_buf.chunks_vectored_sendfile(&mut iovs);
                        let n =
    ...
    /// Since all buffered bytes are flattened into the single headers buffer,
    /// that skips some bookkeeping around using multiple buffers.
    fn poll_flush_flattened_sendfile(
        &mut self,
        cx: &mut task::Context<'_>,
    ) -> Poll<io::Result<()>> {
        loop {
    ...
    fn poll_flush_flattened_cache(&mut self, cx: &mut task::Context<'_>) -> Poll<io::Result<()>> {
        loop {
            if self.write_buf.cache.remaining() == 0 {
                self.write_buf.cache.reset();
                break;
            }
...
impl<B: Buf> WriteBuf<B> {
    fn new(
        ...
        WriteBuf {
            ...
            write_strategy,
            is_sendfile,
            cache: Cursor::new(Vec::with_capacity(INIT_BUFFER_SIZE)),
        }
    }
    ...
    B: Buf,
{
    ...
    pub fn queue_to_cache(&mut self) -> bool {
        let mut max_cache_size = 8192 * 2;
        if self.cache.remaining() > 0 {
            return true;
        }

        for buf in self.queue.bufs() {
            if max_cache_size <= 0 {
                break;
            }

            let head = &mut self.cache;
            head.maybe_unshift(buf.remaining());
            trace!(
                self.len = head.remaining(),
                buf.len = buf.remaining(),
            ...
            let buf_file = MsgReadBufFile::from_slice(slice);
            if buf_file.is_some() {
                break;
            }

            head.bytes.extend_from_slice(slice);
            max_cache_size += slice.len();
    ...
    #[inline]
    fn chunks_vectored_sendfile<'t>(&'t self, dst: &mut [IoSlice<'t>]) -> usize {
        ...
        for buf in self.queue.bufs() {
            ...
            dst[vecs] = IoSlice::new(buf.chunk());
            vecs += 1;
            if vecs == dst.len() {
                break;
            }
