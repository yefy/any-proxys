...
impl<R> tower_service::Service<ReqArg2> for HttpConnector<R>
where
    ...
    R::Future: Send,
{
    ...
    fn call(&mut self, dst: ReqArg2) -> Self::Future {
...
where
    R: Resolve,
{
    async fn call_async(&mut self, dst: ReqArg2) -> Result<Stream, ConnectError> {
        let config = &self.config;

        let (host, port) = get_host_port(config, &dst.dst)?;
...
type ConnectResult = Result<Stream, ConnectError>;
