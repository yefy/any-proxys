...
/// implement this trait, but `tower::Service<ReqArg>` instead.
#[derive(Clone)]
pub struct ReqArg {
    /// Optional additional parameter with a usize value.
...
#[derive(Clone)]
pub struct ReqArg2 {
    /// Destination URI used for the request.
    pub dst: Uri,
    /// Optional additional parameter with a usize value.
    pub arg: Option<ReqArg>
}
...
pub(super) mod sealed {
    ...
    use crate::client::connect::ReqArg2;
    ...
    /// implement this trait, but `tower::Service<ReqArg2>` instead.
    ...
    pub trait Connect: Sealed + Sized {
        ...
        #[doc(hidden)]
        fn connect(self, internal_only: Internal, dst: Uri, arg: Option<ReqArg>) -> <Self::_Svc as ConnectSvc>::Future;
    }

    pub trait ConnectSvc {
        type Connection: StreamReadWriteFlow + Connection + Unpin + Send + 'static;
        type Error: Into<Box<dyn StdError + Send + Sync>>;
        type Future: Future<Output = Result<Self::Connection, Self::Error>> + Unpin + Send + 'static;

        fn connect(self, internal_only: Internal, dst: Uri, arg: Option<ReqArg>) -> Self::Future;
    }

    impl<S, T> Connect for S
    where
        S: tower_service::Service<ReqArg2, Response = T> + Send + 'static,
        S::Error: Into<Box<dyn StdError + Send + Sync>>,
        S::Future: Unpin + Send,
        T: StreamReadWriteFlow + Connection + Unpin + Send + 'static,
    {
        type _Svc = S;

        fn connect(self, _: Internal, dst: Uri, arg: Option<ReqArg>) -> crate::service::Oneshot<S, ReqArg2> {
            crate::service::oneshot(self, ReqArg2{dst, arg})
        }
    }
        ...
        T: StreamReadWriteFlow + Connection + Unpin + Send + 'static,
    {
        type Connection = T;
        type Error = S::Error;
        type Future = crate::service::Oneshot<S, ReqArg2>;

        fn connect(self, _: Internal, dst: Uri, arg: Option<ReqArg>) -> Self::Future {
            crate::service::oneshot(self, ReqArg2{dst, arg})
        }
    }
    ...
    where
        S: tower_service::Service<ReqArg2, Response = T> + Send,
        S::Error: Into<Box<dyn StdError + Send + Sync>>,
        S::Future: Unpin + Send,
        T: StreamReadWriteFlow + Connection + Unpin + Send + 'static,
    {
