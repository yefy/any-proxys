...
type BodySender = mpsc::Sender<Result<MsgWriteBuf, crate::Error>>;
...
enum Kind {
    Once(Option<MsgWriteBuf>),
    Chan {
        content_length: DecodedLength,
        want_tx: watch::Sender,
        data_rx: mpsc::Receiver<Result<MsgWriteBuf, crate::Error>>,
...
impl Body {
    ...
    fn poll_eof(&mut self, cx: &mut task::Context<'_>) -> Poll<Option<crate::Result<MsgWriteBuf>>> {
    ...
    ) -> Poll<Option<crate::Result<MsgWriteBuf>>> {
        match self.kind {
            Kind::Once(ref mut val) => Poll::Ready(val.take().map(|bytes| Ok(bytes))),
            Kind::Chan {
            ...
            Kind::H2 {
                ...
                Some(Ok(bytes)) => {
                    ...
                    Poll::Ready(Some(Ok(MsgWriteBuf::from_bytes(bytes))))
                }
            ...
            Kind::Wrapped(ref mut s) => match ready!(s.get_mut().as_mut().poll_next(cx)) {
                Some(res) => Poll::Ready(Some(
                    res.map_err(crate::Error::new_body)
                        .and_then(|data| Ok(MsgWriteBuf::from_bytes(data))),
                )),
                None => Poll::Ready(None),
            },
        }
    ...
    #[cfg(feature = "http1")]
    pub(super) fn take_full_data(&mut self) -> Option<MsgWriteBuf> {
...
impl Stream for Body {
    type Item = crate::Result<MsgWriteBuf>;
...
impl From<Bytes> for Body {
    ...
    fn from(chunk: Bytes) -> Body {
        if chunk.is_empty() {
            ...
            Body::new(Kind::Once(Some(MsgWriteBuf::from_bytes(chunk))))
        }
    }
}
...
impl Sender {
    ...
    pub async fn send_data(&mut self, chunk: MsgWriteBuf) -> crate::Result<()> {
        self.ready().await?;
        self.data_tx
            .try_send(Ok(chunk))
            .map_err(|_| crate::Error::new_closed())
    ...
    /// `send_data()` instead.
    pub fn try_send_data(&mut self, chunk: MsgWriteBuf) -> Result<(), MsgWriteBuf> {
        self.data_tx
            .try_send(Ok(chunk))
