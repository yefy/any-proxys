...
impl Upgraded {
    ...
    where
        T: StreamReadWriteFlow + Unpin + Send + 'static,
    {
        Upgraded {
    ...
    /// `Upgraded` back.
    pub fn downcast<T: StreamReadWriteFlow + Unpin + 'static>(self) -> Result<Parts<T>, Self> {
...
pub(super) trait Io: StreamReadWriteFlow + Unpin + 'static {
    fn __hyper_type_id(&self) -> TypeId {
        TypeId::of::<Self>()
    }
}

impl<T: StreamReadWriteFlow + Unpin + 'static> Io for T {}
