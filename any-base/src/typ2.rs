...
lazy_static! {
    pub static ref TOKIO_LOCK_INDEX_MAP: Arc<parking_lot::Mutex<HashMap<String, i64>>> =
...
impl<'a, T: 'a> ArcMutexReadGuardTokio<'a, T> {
    pub fn new(
        d: tokio::sync::MutexGuard<'a, Option<T>>,
        #[cfg(any(debug_assertions, feature = "anylock-time"))] key: String,
    ) -> Self {
...
impl<'a, T: 'a> Drop for ArcMutexReadGuardTokio<'a, T> {
    ...
    fn drop(&mut self) {
        #[cfg(any(debug_assertions, feature = "anylock-time"))]
        {
            TOKIO_LOCK_INDEX_MAP.lock().remove(&self.key);
        }
    }
}
...
impl<T> ArcMutexTokio<T> {
    ...
    pub async fn get(&self, _file: &str, _line: u32) -> ArcMutexReadGuardTokio<T> {
        #[cfg(any(debug_assertions, feature = "anylock-time"))]
        {
            let key = format!("{}:{}", _file, _line);
            let time = Local::now().timestamp_micros();
            TOKIO_LOCK_INDEX_MAP.lock().insert(key.clone(), time);
            ArcMutexReadGuardTokio::new(self.d.lock().await, key)
        }
    ...
    pub async fn get_mut(&self, _file: &str, _line: u32) -> ArcMutexWriteGuardTokio<T> {
        #[cfg(any(debug_assertions, feature = "anylock-time"))]
        {
            ...
            let time = Local::now().timestamp_micros();
            TOKIO_LOCK_INDEX_MAP.lock().insert(key.clone(), time);
            ArcMutexWriteGuardTokio::new(self.d.lock().await, key)
        }
...
impl<T> ArcRwLockTokio<T> {
    ...
    pub async fn get(&self, _file: &str, _line: u32) -> ArcRwLockReadTokioGuard<T> {
        #[cfg(any(debug_assertions, feature = "anylock-time"))]
        {
            let key = format!("{}:{}", _file, _line);
            let time = Local::now().timestamp_micros();
            TOKIO_LOCK_INDEX_MAP.lock().insert(key.clone(), time);
            ArcRwLockReadTokioGuard::new(self.d.read().await, key)
        }

        #[cfg(not(any(debug_assertions, feature = "anylock-time")))]
        ArcRwLockReadTokioGuard::new(self.d.read().await)
    }
    pub async fn get_mut(&self, _file: &str, _line: u32) -> ArcRwLockWriteTokioGuard<T> {
        #[cfg(any(debug_assertions, feature = "anylock-time"))]
        {
            let key = format!("{}:{}", _file, _line);
            let time = Local::now().timestamp_micros();
            TOKIO_LOCK_INDEX_MAP.lock().insert(key.clone(), time);
            ArcRwLockWriteTokioGuard::new(self.d.write().await, key)
        }
