...
impl<'a, T: 'a> Drop for ArcMutexReadGuard<'a, T> {
    ...
    fn drop(&mut self) {
        ...
        #[cfg(any(debug_assertions, feature = "anylock-time"))]
        {
            let is_check_time = self.info.lock().unwrap().is_check_time;
            if is_check_time && self.start_time.elapsed().as_millis() > MAX_ELAPSED_TIME_MIL {
                panic!("lock panic");
            }
        }
    }
...
impl<T> ArcMutex<T> {
    ...
    fn get_write_lock(&self) -> MutexGuard<'_, Option<T>> {
        ...
        {
            return self.d.lock().unwrap();
        }

        #[cfg(any(debug_assertions, feature = "anylock-time"))]
        loop {
            let thread_id = std::thread::current().id();
            let info = &mut *self.info.lock().unwrap();
            ...
            if let Err(e) = &lock {
                if let &TryLockError::WouldBlock = e {
                    continue;
                }
            }
