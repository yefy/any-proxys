...
use crate::rt::LocalExec;
use crate::rt::ThreadExec;
...
impl<E: Clone> ExecutorsSpawn<E> {
    ...
    where
        S: FnOnce(ExecutorsSpawn<E>) -> F + 'static,
        F: Future<Output = Result<()>> + 'static,
        E: SpawnExec<F>,
    {
        _start(true, self.clone(), service)
    }
    ...
    where
        ...
        E: SpawnExec<F>,
    {
        _start(false, self.clone(), service)
    }
...
impl<E: Clone> ExecutorSpawn<E> {
    ...
    where
        ...
        E: SpawnExec<F>,
    {
        _start(true, self.executors.clone(), service)
    }
    ...
    where
        ...
        E: SpawnExec<F>,
    {
        _start(false, self.executors.clone(), service)
    }
...
pub fn _block_on_local<S, F>(worker_threads_blocking: usize, service: S) -> Result<()>
where
    S: FnOnce(ExecutorSpawn<LocalExec>) -> F + 'static,
    F: Future<Output = Result<()>> + 'static,
{
    _block_on(1, worker_threads_blocking, LocalExec, service)
}

pub fn _block_on_thread<S, F>(
    thread_num: usize,
    worker_threads_blocking: usize,
    service: S,
) -> Result<()>
where
    S: FnOnce(ExecutorSpawn<ThreadExec>) -> F + 'static,
    F: Future<Output = Result<()>> + 'static + Send,
{
    _block_on(thread_num, worker_threads_blocking, ThreadExec, service)
}

pub fn _block_on<S, F, E>(
    thread_num: usize,
    worker_threads_blocking: usize,
    executor: E,
    service: S,
...
where
    S: FnOnce(ExecutorSpawn<E>) -> F + 'static,
    F: Future<Output = Result<()>> + 'static,
    E: SpawnExec<F>,
{
    if thread_num > 1 {
