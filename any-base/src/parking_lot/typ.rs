...
impl<T> ArcMutex<T> {
    ...
    pub fn get(&self) -> ArcMutexReadGuard<T> {
        ArcMutexReadGuard::new(self.d.lock())
    }
    pub fn get_mut(&self) -> ArcMutexWriteGuard<T> {
        ArcMutexWriteGuard::new(self.d.lock())
    }

    pub fn get_option(&self) -> ArcMutexReadOptionGuard<Option<T>> {
        ArcMutexReadOptionGuard::new(self.d.lock())
    }
    pub fn get_option_mut(&self) -> ArcMutexWriteOptionGuard<Option<T>> {
        ArcMutexWriteOptionGuard::new(self.d.lock())
    }

    pub unsafe fn take(&self) -> T {
        self.d.lock().take().unwrap()
    }
}
...
impl<T> ArcMutexBox<T> {
    ...
    pub fn set(&self, d: T) {
        *self.d.lock() = Some(Box::new(d));
    }

    pub fn get(&self) -> ArcMutexReadGuard<Box<T>> {
        ArcMutexReadGuard::<Box<T>>::new(self.d.lock())
    }
    pub fn get_mut(&self) -> ArcMutexWriteGuard<Box<T>> {
        ArcMutexWriteGuard::<Box<T>>::new(self.d.lock())
    }
...
impl<T> ArcRwLock<T> {
    ...
    pub fn get(&self) -> ArcRwLockReadGuard<T> {
        ArcRwLockReadGuard::new(self.d.read())
    }
    pub fn get_mut(&self) -> ArcRwLockWriteGuard<T> {
        ArcRwLockWriteGuard::new(self.d.write())
    }

    pub unsafe fn take(&self) -> T {
        self.d.write().take().unwrap()
    }
}
