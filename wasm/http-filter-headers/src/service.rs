...
pub fn wasm_main(_config: Option<String>) -> Result<wasm_std::Error> {
    ...
    wasm_std::out_add_headers(&vec![("server".to_string(), version)]).map_err(|e| anyhow::anyhow!("{}", e))?;
    let request_id = wasm_std::variable("request_id").map_err(|e| anyhow::anyhow!("{}", e))?;
    if request_id.is_some() {
        let request_id = request_id.unwrap();
        wasm_std::out_add_header("anyproxy_request_id", &request_id).map_err(|e| anyhow::anyhow!("{}", e))?;
    }
